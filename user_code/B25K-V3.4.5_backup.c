...
interrupt [EXT_INT6] void ext_int6_isr(void)
{
    ...
    //if(phase_a_cnt>2){    
    if(PHASE_A==LEVEL_HI){
        if(phase_z_cnt==0 ){                             //전원이 ON된후 처음 시작한 뒤, z펄스가 들어오면 클리어
        ...
        else if( phase_a_cnt >= SYS_INFO.z_encoder_interval && phase_z_cnt>=1){       //a상 카운터값이 엔코더 max 카운터보다 크면 클리어
...
interrupt [EXT_INT7] void ext_int7_isr(void)
{
    ...
    if(phaseB==CW_DIR){    //알찬 & 공장용 엔코더 :: 우방향  :: CW_DIR = 1
    //if(phaseB==0){      //상지 집란장 :: 좌방향 :: CW_DIR = 0
        ...
        switch(modRunning){
            ...
            case NORMAL_RUN :
                ...
                if(MEASURE_INFO.crackchecktime <=phase_a_cnt && phase_a_cnt < crackchk_end_pulse ){
                    if(Crack_Dot_Chk_Enable==1){//크랙디텍터, 오란검출기 신호 체크
                        crackValue = EDI_CH0;
                        dotValue = EDI_CH1;
                        ...
                        for(lcnt=0;lcnt<6;lcnt++){
                            ...
                            if((crackValue & crack_dot_ch)==crack_dot_ch){ //각 채널별 crack신호가 있으면
                                crackCHKCNT[lcnt]++;                                                     
                            }
                            //오물란 신호 감지 일시중지
...
void WeightGrading(unsigned char ch){
    ...
    if((crackdata == 0 ) && dotCHKCNT[ch]<= 5){     //크랙이나 오물란 신호가 없으면 :: 5이하이면 무시
        //crackCHKCNT[ch] = 0;
        //crack data reset;
        ...
        dotCHKCNT[ch] = 0;
        //Grading 
        if(gradeWeight >= GRADE_INFO[g_ETC].hiLimit){       //상등외     
        ...
        else if((gradeWeight < GRADE_INFO[g_ETC].hiLimit)&&(gradeWeight >= GRADE_INFO[g_KING].loLimit)){ //왕란
            grade_result = g_KING;   
        ...
        else if((gradeWeight < GRADE_INFO[g_KING].loLimit)&&(gradeWeight >= GRADE_INFO[g_SPECIAL].loLimit)){ //특란::왕란 하한보다 작고, 특란 하한보다 크거나 같은 경우 
        ...
        else if((gradeWeight < GRADE_INFO[g_SPECIAL].loLimit)&&(gradeWeight >=GRADE_INFO[g_BIG].loLimit)){ //대란::특란 하한보다 작고, 대란 하한보다 크거나 같은 경우
            grade_result = g_BIG;       
        ...
        else if((gradeWeight < GRADE_INFO[g_BIG].loLimit)&&(gradeWeight >= GRADE_INFO[g_MIDDLE].loLimit)){ //중란::대란 하한보다 작고, 중란 하한보다 크거나 같은 경우 
        ...
        else if((gradeWeight < GRADE_INFO[g_MIDDLE].loLimit)&&(gradeWeight >= GRADE_INFO[g_SMALL].loLimit)){ //소란 :: 중란 하한보다 작고, 소란 하한보다 크거나 같은 경우
        ...
        else if((gradeWeight < GRADE_INFO[g_SMALL].loLimit)&&(gradeWeight >= GRADE_INFO[g_LIGHT].loLimit)){ //경란 :: 소란 하한보다 작고, 경란 하한보다 크거나 같은 경우
        ...
        else if( (gradeWeight < GRADE_INFO[g_LIGHT].loLimit) && (gradeWeight >= 100)){       //하등외 :: 경란의 하한보다 작고, 10.0g보다 크거나 같은 경우
            grade_result = g_ETC;   
