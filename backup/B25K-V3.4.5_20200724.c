...
unsigned char LifeTime_Start_Flag = FALSE, LifeTime_End_Flag = FALSE;
...
interrupt [EXT_INT0] void ext_int0_isr(void)
{
    ...
    if(SpeedAdjEnable == TRUE)
    {
        //슬라이드 모터 OFF
        if(PIND.0 == TRUE && slidemotor_state == TRUE && accel_chk == TRUE && MACHINE_STATE==TRUE )
        {
         //가속종료
            if(debugMode==1)
            {
            ...
            AccelCount=accelStartCount; //    accelStartCount => 3
            Sol_Correction_Count = 0;

            SpeedAdjEnable = FALSE;
            slidemotor_state = FALSE;
            accel_chk = FALSE;
            //deaccel_rdy = 1;
         }
        ...
        //슬라이드모터ON
        else if(PIND.0 == FALSE && slidemotor_state == FALSE && MACHINE_STATE == FALSE && accel_rdy == TRUE)
        {
            if(debugMode==1)
            {
                UDR1 =0xA0;
            ...
            slidemotor_state = TRUE;
            accel_rdy = FALSE;
            accel_chk = TRUE;
            timerstop = 0;
            AccelCount =0;
            AccelStep = accelStartCount;   //  accelStartCount => 3

            MACHINE_STATE = TRUE;
            off_Correction_Count = init_off_correction; //init_off_correction => 25
            Sol_Correction_Count=SYS_INFO.correction+AccelStartPos;  //60 + 5
            AccelStart = TRUE;
            TimerMode = tmrSPEED;
            OCR1AH=0x00;
            OCR1AL=0x4E;        //5msec /1024 prescaler
            TCCR1B = 0x05;
        }
    }
...
interrupt [EXT_INT1] void ext_int1_isr(void)
{
    ...
    {
        //슬라이드 모터 OFF
        if(PIND.1 == TRUE && slidemotor_state == TRUE && MACHINE_STATE == FALSE && deaccel_chk == TRUE )
        {
           if(debugMode==1)
           {
                UDR1 =0xB1;
           ...
           DeAccelStart = TRUE;           //감속종료
           DeAccelStep  = deAccelStartCount; //  accelStartCount => 5
           DeAccelCount = 0;
        ...
        //슬라이드모터ON
        else if(PIND.1 == FALSE && slidemotor_state == FALSE && MACHINE_STATE == TRUE && deaccel_rdy == TRUE)
        {
            //감속시작
...
//     TxBuffer[0] = 0x02;
//     TxBuffer[1] = 0x00;
//     TxBuffer[2] = LifeTime_Start_Flag;
//     TxBuffer[3] = LifeTime_End_Flag;
//     TxBuffer[4] = 0x03;
//     TxEnable = 1;   txcnt = 1;  TxLength = 5;   UDR1 = TxBuffer[0];

    if(LifeTime_Start_Flag != LifeTime_End_Flag)
    {
         TxBuffer[0] = 0x02;
         TxBuffer[1] = 0x00;
         TxBuffer[2] = 0x01;
         TxBuffer[3] = 0xA2;
         TxBuffer[4] = 0x03;
         TxEnable_Event = 1;   txcnt = 1;  TxLength = 5;   UDR1 = TxBuffer[0];
    }
    ...
    if(LifeTime_Start_Flag == LifeTime_End_Flag)
    {
         ...
         TxBuffer[4] = 0x03;
         TxEnable_Event = 1;   txcnt = 1;  TxLength = 5;   UDR1 = TxBuffer[0];
         LifeTime_Start_Flag = FALSE;
         LifeTime_End_Flag = FALSE;
    }
...
interrupt [EXT_INT6] void ext_int6_isr(void)
{
    ...
    //ACD_DATA_BUF[0]가 가리키는 곳부터  tmp_acd_data[0]가 가리키는 곳에 복사(ACD_DATA_BUF 크기)
    ...
    if(PHASE_A==LEVEL_HI)
    {
        ...
        //tmp_acd_data[0]가 가리키는 곳부터 ACD_DATA_BUF[0]가 가리키는 곳에 복사(ACD_DATA_BUF -1 크기)
        ACD_DATA_BUF[0]=0;
    }
    LED_BUZ = led_buz_value;
}
...
interrupt [EXT_INT7] void ext_int7_isr(void)
{
    ...
    if(phaseB==CW_DIR)
    {
        ...
        //이벤트 신호 입력시 정지
        if(EVENT_ENABLE == TRUE && phase_a_cnt == SYS_INFO.stopcount)
        {
            plc_prt_value |= EVENT_STOP;
            PLC_CON = plc_prt_value;
...
interrupt [USART1_TXC] void usart1_tx_isr(void)
{
    if(TxEnable_Event)
    {
        ...
        if(txcnt==TxLength)
        {
            for(Tx_Cnt = 1; Tx_Cnt == txcnt; Tx_Cnt++)
            {
                UDR1 = TxBuffer[Tx_Cnt];
            }

            TxEnable_Event = 0;   txcnt = 0;      A_CNT_TX_HOLD = 0;
        }
    ...
    switch(TimerMode)
    {
        ...
        case tmrSPEED : // 슬라이드 Control
            if(modRunning == NORMAL_RUN || modRunning == SOLENOID_TEST)
            {
                ...
                {
                    AccelCount++;
                    if((AccelCount % unit_rising) == FALSE)
...
void CorrectionData(unsigned char ch){
    ...
    {
        if(voltData[ch][lcnt] >= maxdata)
        {
         maxdata=voltData[ch][lcnt];
        }

        if(voltData[ch][lcnt] <= mindata)
        {
         mindata=voltData[ch][lcnt];
        }
...
void ConvertWeight(unsigned char ch)
{
    ...
    if(debugMode==2){
        ...
        UDR1 =weight_value[ch]>>8;
        while(!(UCSR1A & 0x20));
        UDR1 =weight_value[ch];
        while(!(UCSR1A & 0x20));
        UDR1 =zero_value[ch]>>8;
        while(!(UCSR1A & 0x20));
        UDR1 =zero_value[ch];
        while(!(UCSR1A & 0x20));
        UDR1 =0xDD;
...
void WeightGrading(unsigned char ch){
    ...
    if(debugMode==3)
    {
        ...
        UDR1 =ACD_DATA_BUF[3];
        while(!(UCSR1A & 0x20));
        UDR1 =ACD_DATA_BUF[SYS_INFO.ACD_CHK_POS];
        while(!(UCSR1A & 0x20));
        UDR1 =0xFB;
        while(!(UCSR1A & 0x20));
    }
    ...
    if((crackdata == 0 ) && dotCHKCNT[ch]<= 5)
    {
        ...
        if(gradeWeight >= GRADE_INFO[g_ETC].hiLimit)
        {
            //상등외
            grade_result = g_ETC;
            GRADE_DATA.gNumber[g_ETC]++;    //등급 저장, 계란 수량 누적
            ...
            if(PackerSharedEnable != 9)
            {
                ...
                else
                {
                    ...
                    if(debugMode==3)
                    {
                        UDR1 =grade_result;
                        while(!(UCSR1A & 0x20));
                        UDR1 =set_bucket_data>>8;
                        while(!(UCSR1A & 0x20));
                        UDR1 =set_bucket_data;
                        while(!(UCSR1A & 0x20));
                        UDR1 =0xDF;
                        while(!(UCSR1A & 0x20));
                    }
...
void Initialize_SYSTEM(void){
    ...
    else{       //Device ID Check Fail :: 공장 초기값으로 데이터 로드
        Initailize_PARAMETER();
    }
            ...
            switch(RxBuffer[0])
            {
                case CMD_REFORM_START :
                    ...
                    */
                    ...
                    TCNT1L = 0x00;
                    OCR1AH=0x01;
                    OCR1AL=0x38;        //20msec /1024 prescaler
                    TCCR1B = 0x05;
                    break;
                ...
                case CMD_REFORM_DATA :
                    ...
                    TxBuffer[1] = 0x00;
                    TxBuffer[2]= 11;
                    TxBuffer[3] = CMD_REFORM_DATA;
                    TxBuffer[4] = reformCH;
                    TxBuffer[5] = reformSigned;
                    TxBuffer[6] = reformVoltage >> 8;
                    TxBuffer[7] = reformVoltage;
                    TxBuffer[8] = reformWeight >> 8;
                    TxBuffer[9] = reformWeight;
                    TxBuffer[10] = zero_value[reformCH] >> 8;
                    TxBuffer[11] = zero_value[reformCH];
                    TxBuffer[12] = ConstWeight[reformCH] >> 8;
                    TxBuffer[13] = ConstWeight[reformCH];
                    TxBuffer[14] = 0x03;
                ...
                case CMD_REFORM_ZERO :
                    ...
                    TCNT1L = 0x00;
                    OCR1AH=0x00;
                    OCR1AL=0x1F;        //2msec /1024 prescaler
                    TCCR1B = 0x05;
                    break;
